...
Gereksinimler:
  ...
  Model indirilmiş olmalı: ollama pull gemma3:12b

Kullanım:
  python profil_maker_gemma3.py --input /path/to/logs --output /path/to/output
  python profil_maker_gemma3.py --input /path/to/logs --output /path/to/output --model gemma3:12b
"""
...
SYSTEM_PROMPT = """You are an information extraction model for PAM (Privileged Access Management) Windows session logs.

Read the event list and return exactly one JSON object with these keys:
- "urls"
- "commands"
- "apps"
- "summary"
...
Rules for "urls":
- Include only real websites that were actually visited in a browser.
- Use only the site name, not the full page title.
- Good examples: "Haberler.com", "YouTube", "ChatGPT", "Haber 7"
- Do not include local folders, application names, window names, browser UI pages, search result pages, ad/tracker pages, "Program Manager", or "Search".

Rules for "commands":
- Include only commands that were actually executed in a terminal.
- Use the command text itself.
- Good examples: "python hesap.py", "python --version", "whoami", "ipconfig", "netstat -ano", "tasklist /svc", "ping 10.0.1.55"
- Do not include source code, search queries, text typed into chat/browser/editor, program input values, file names, folder names, rename text, or terminal window titles.
- Do not invent commands.
- Do not output commands with count 0.

Rules for "apps":
- Include only desktop applications that were actively used.
- Normalize names to clean app names.
- Good examples: "Google Chrome", "Command Prompt", "File Explorer", "Python", "MobaXterm", "WinRAR", "FortiClient"
- Do not include "Program Manager", "Search", "Server Manager", "Rename", "Progress", "ShellExperienceHost", or raw window titles.
...
def classify_with_llm(
    ...
    for attempt in range(max_retries):
        try:
            # Gemma resmi dökümana göre system rolü desteklemez;
            # talimatları ilk user mesajına göm.
            actual_prompt = (
                SYSTEM_PROMPT
                + "\n\nSession events JSON:\n"
                + user_content
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Örnekler:
  # Varsayılan (Gemma 3 12B, Ollama)
  python profil_maker_gemma3.py --input ./logs --output ./analyzed

  # Farklı model
  python profil_maker_gemma3.py --input ./logs --output ./analyzed --model gemma3:12b

  # Farklı endpoint (LM Studio, vLLM vb.)
  python profil_maker_gemma3.py --input ./logs --output ./analyzed --endpoint http://localhost:1234/v1

  # Tek dosya
  python profil_maker_gemma3.py --input ./logs/session1.json --output ./analyzed
        """,
    )
    ...
    parser.add_argument("--model", "-m", default="gemma3:12b",
                        help="Model adı (varsayılan: gemma3:12b)")
