...
# URLs
Extract website names from browser activity, especially from browser window titles.
Use the website, platform, publisher, service, or web source name, not the full article title, page title, local folder name, or application name.

Do not require "http", "https", ".com", or any domain suffix to identify a website.
If a browser window clearly represents a web page, search page, article page, results page, or online service page visited by the user, extract the website name even when the title does not contain a full URL.

If a browser title contains both page content and a site or source name, extract the site or source name.
If the exact website name is not written as a domain but the title clearly indicates a recognizable website, search engine, platform, publisher, service, or web source, extract that website name.
When multiple nearby browser events belong to the same browsing flow, infer the website name from the most explicit nearby browser title and reuse it consistently for related page views.
For browser events, prefer identifying the visited website over returning an empty URL list.

Do NOT include:
- local folders or file system locations
- desktop applications such as browser names, terminal names, or OS tools
- generic browser UI labels such as "New Tab", "History", or "Downloads" unless they clearly indicate a visited website
- OS windows such as "Program Manager" or "Search"
- arbitrary typed text unless it clearly corresponds to a visited website
